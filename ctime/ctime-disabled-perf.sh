...
#Disable ctime
gluster vol set master ctime off

#Mount the volume
mount -t glusterfs hrk:master /cd-mastermnt

#Start profile
gluster vol profile master start

#Run perf test
/root/sandbox/upstream/smallfile/smallfile_cli.py --stonewall N --operation create --threads 4 --file-size 64 --files 16000 --top /cd-mastermnt --host-set hrk --output-json /root/smallfile_results/ctime_disabled/downstream/run${run}/create

<<STATEDUMP_CREATE
#Get Statedump
client_pid=$(ps -ef | grep glusterfs | grep -F "cd-mastermnt" | awk '{print $2}')
...
setfattr -n trusted.io-stats-dump -v create-cd-mastermnt /cd-mastermnt
gluster vol profile master info > /root/smallfile_results/ctime_disabled/downstream/run${run}/create_profile

#Clear profile info
gluster vol profile master info clear

/root/sandbox/upstream/smallfile/smallfile_cli.py --stonewall N --operation rename --threads 4 --file-size 64 --files 16000 --top /cd-mastermnt --host-set hrk --output-json /root/smallfile_results/ctime_disabled/downstream/run${run}/rename
...
setfattr -n trusted.io-stats-dump -v rename-cd-mastermnt /cd-mastermnt
gluster vol profile master info > /root/smallfile_results/ctime_disabled/downstream/run${run}/rename_profile
